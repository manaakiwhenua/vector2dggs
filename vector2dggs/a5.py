...
@click.option(
    ...
    required=False,
    type=click.Choice(list(map(str, range(const.MIN_A5, const.MAX_A5 + 1)))),
    help="A5 parent resolution for the output partition. Defaults to resolution - 6",
)
@click.option(
    "-id",
    "--id_field",
...
@click.option(
    ...
    help="Retain attributes in output. The default is to create an output that only includes A5 cell ID and the ID given by the -id field (or the default index ID).",
)
@click.option(
    "-ch",
    "--chunksize",
...
@click.option(
    ...
    "--compact",
    is_flag=True,
    help="Compact the A5 cells up to the parent resolution. Compaction requires an id_field.",
)
...
def a5(
    ...
    """
    Ingest a vector dataset and index it to the A5 DGGS.
